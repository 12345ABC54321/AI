...
// Lance le modèle et retourne l’index de la classe prédite
int ai_predict(void)
{
    if (!interpreter || !output_tensor) {
        ESP_LOGE(TAG, "ai_predict() appelé avant ai_init()");
        return -1;
    }

    TfLiteStatus status = interpreter->Invoke();
    if (status != kTfLiteOk) {
        ESP_LOGE(TAG, "Invoke() a échoué");
        return -1;
    }

    float *out = output_tensor->data.f;

    // On suppose un tenseur [1, N] ou [N]
    int num_classes;
    if (output_tensor->dims->size == 1) {
        num_classes = output_tensor->dims->data[0];
    } else {
        num_classes = output_tensor->dims->data[output_tensor->dims->size - 1];
    }
    ...
    ESP_LOGI(TAG, "Classe prédite = %d (score=%f)", best_idx, best_val);
    return best_idx;
}
